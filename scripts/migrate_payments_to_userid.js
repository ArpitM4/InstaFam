...
async function migrateFullPayments() {
  ...
  console.log(`🔎 Found ${paymentsToMigrate.length} payments to migrate (out of ${totalPayments} total).`);
  ...
  const migratedPayments = await Payment.countDocuments({ from_user: { $exists: true } });
  console.log('\n--- MIGRATION COMPLETE ---');
  console.log(`✅ Successfully Migrated: ${migrated}`);
  console.log(`❌ Failed/Skipped: ${errors}`);
  console.log(`Total payments in DB: ${totalPayments}`);
  console.log(`Payments with from_user after migration: ${migratedPayments}`);
