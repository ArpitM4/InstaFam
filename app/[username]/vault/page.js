...
const UsernamePage = async ({ params }) => {
  ...
  // Check if vault section is visible
  const visibleSections = user.visibleSections || ['contribute', 'vault', 'links'];

  if (!visibleSections.includes('vault')) {
    if (isOwnProfile) {
  ...
  return <PaymentPage username={params.username} initialUser={user} initialVaultItems={vaultItems} initialTab="vault" />;
};
