...
const UsernamePage = async ({ params }) => {
  ...
  // Check if contribute section is visible
  const visibleSections = user.visibleSections || ['contribute', 'vault', 'links'];

  if (!visibleSections.includes('contribute')) {
    if (isOwnProfile) {
  ...
  return <PaymentPage username={params.username} initialUser={user} initialVaultItems={vaultItems} initialTab="contribute" />;
};
