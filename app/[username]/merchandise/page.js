...
const UsernamePage = async ({ params }) => {
  ...
  // Check if merchandise section is visible
  const visibleSections = user.visibleSections || ['contribute', 'vault', 'links'];

  if (!visibleSections.includes('merchandise')) {
    if (isOwnProfile) {
  ...
  return <PaymentPage username={params.username} initialUser={user} initialVaultItems={vaultItems} initialTab="merchandise" />;
};
